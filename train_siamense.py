...
#plt.ion()
import sys
#sys.path.insert(0,'/share/apps/py-faster-rcnn/20160617/gnu/python')
#sys.path.insert(0,'/share/apps/py-faster-rcnn/20160617/gnu/python/caffe')
import caffe
import numpy as np
#import pdb
...
if __name__=='__main__':
    ...
    #caffe.set_device(0)
    #caffe.set_mode_gpu()
    solver = caffe.SGDSolver('auto_solver_siamense.prototxt')
   ...
   # solver.net.copy_from('/scratch/ys1297/caffe-installation/model/verizon/')
    ...
    with open('train_siamense.txt','w') as txt_file:
    	for it in range(niter):
            solver.step(1)  # SGD by Caffe
#            pdb.set_trace()
 	    train_loss[it] = solver.net.blobs['loss'].data
	    #print 'train_loss'+ str(train_loss[it])
	    print 'train_loss'+ str(train_loss[it]) + " std :" + str(solver.net.params['fc6'][0].data.std()) +"iteration: " +str(it)
		...
		txt_file.write('iteration: '+ str(it)+  ' test_loss: '+str(loss) +'\n')		
           	solver.net.save('../model/siamense_new_%f'%(it))
